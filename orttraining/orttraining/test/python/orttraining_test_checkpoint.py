...
def _single_run(execution_file, scenario, checkopint_dir):
    ...
    assert subprocess.call([sys.executable, execution_file, '--scenario', scenario, '--checkpoint_dir', checkopint_dir], env={'PYTHONPATH': working_directory}) == 0

def _distributed_run(execution_file, scenario, checkopint_dir):
    working_directory = os.path.dirname(os.path.abspath(__file__))
    assert subprocess.call(['mpirun', '-n', str(ngpus), '-x', 'NCCL_DEBUG=INFO', sys.executable, execution_file, '--scenario', scenario,  '--checkpoint_dir', checkopint_dir], env={'PYTHONPATH': working_directory}) == 0
